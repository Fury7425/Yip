...
namespace
{
    ...
    StateGeom GeometryFor(::yip::IndicatorState s) noexcept
    {
        ...
        switch (s) {
            case S::Idle:      return { 92.0f,  28.0f, 0.60f };
            case S::Armed:     return { 132.0f, 36.0f, 1.00f };
            case S::Recording: return { 188.0f, 44.0f, 1.00f };
            case S::Saving:    return { 188.0f, 44.0f, 0.85f };
            case S::Expanded:  return { 320.0f, 56.0f, 1.00f };
        }
...
namespace winrt::yip::implementation
{
    IndicatorWindow::IndicatorWindow()
    {
        ...
        if (m_hwnd) {
            HRGN rgn = ::CreateRoundRectRgn(
                0, 0, kWindowW + 1, kWindowH + 1,
                static_cast<int>(kCorner * 2), static_cast<int>(kCorner * 2));
    ...
    void IndicatorWindow::SyncFromAudioCore()
    {
        ...
        if (recording) {
            ...
            const auto since = std::chrono::duration<float, std::milli>(
                now - m_lastRecordingTrueTs).count();
    ...
    void IndicatorWindow::TransitionTo(::yip::IndicatorState s, bool animate)
    {
        ...
        const bool wantActions =
            (s == ::yip::IndicatorState::Expanded);
        const bool wantMeter =
            (s == ::yip::IndicatorState::Recording ||
             s == ::yip::IndicatorState::Expanded ||
        ...
        if (!meterShouldRun && m_meterTimer.IsRunning())       { m_meterTimer.Stop(); StopMeterAnimations(); }

        // Auto-collapse after 3s when expanded.
        if (s == ::yip::IndicatorState::Expanded) ResetAutoCollapseTimer();
        else                                       StopAutoCollapseTimer();
    ...
    void IndicatorWindow::OnPillPointerPressed(
    ...
    void IndicatorWindow::OnPillPointerMoved(
        winrt::Windows::Foundation::IInspectable const& /*sender*/,
        muxi::PointerRoutedEventArgs const& args)
    {
    ...
    void IndicatorWindow::OnPillPointerReleased(
        winrt::Windows::Foundation::IInspectable const& /*sender*/,
        muxi::PointerRoutedEventArgs const& args)
    {
    ...
    void IndicatorWindow::OnPillPointerCaptureLost(
        winrt::Windows::Foundation::IInspectable const& /*sender*/,
        muxi::PointerRoutedEventArgs const& /*args*/)
    {
        m_dragging = false;
    ...
    void IndicatorWindow::OnPillTapped(
        winrt::Windows::Foundation::IInspectable const& /*sender*/,
    ...
    void IndicatorWindow::OnStopClicked(
        winrt::Windows::Foundation::IInspectable const& /*sender*/,
    ...
    void IndicatorWindow::OnMarkClicked(
        winrt::Windows::Foundation::IInspectable const& /*sender*/,
        mux::RoutedEventArgs const& /*args*/)
    {
    ...
    void IndicatorWindow::OnOpenLastClicked(
        winrt::Windows::Foundation::IInspectable const& /*sender*/,
        mux::RoutedEventArgs const& /*args*/)
    {
        std::wstring target;
        ...
        if (target.empty()) {
            ...
            for (auto const& e : std::filesystem::directory_iterator(root, ec)) {
                ...
                if (bestPath.empty() || t > best) { best = t; bestPath = e.path(); }
            }
    ...
    void IndicatorWindow::RestoreFromPersistence()
    {
        ...
        for (auto const& d : displays) {
            if (d.DisplayId().Value == m_persisted.monitor_id) { chosen = d; break; }
        }
        if (!chosen) {
        ...
        switch (m_persisted.dock_edge) {
            case ::yip::DockEdge::Top:
                x = work.X + static_cast<int>(std::lround(
                    (work.Width  - kWindowW) * t));
                y = work.Y + 12;
                break;
            ...
            case ::yip::DockEdge::Left:
                x = work.X + 12;
                y = work.Y + static_cast<int>(std::lround(
                    (work.Height - kWindowH) * t));
                break;
    ...
    void IndicatorWindow::SnapToNearestEdgeIfClose()
    {
        ...
        int x = rc.left;
        int y = rc.top;
        if (minDist == distTop)         y = work.Y + 12;
        else if (minDist == distBottom) y = work.Y + work.Height - kWindowH - 12;
        else if (minDist == distLeft)   x = work.X + 12;
        else if (minDist == distRight)  x = work.X + work.Width - kWindowW - 12;
        appWindow.Move({ x, y });
    }
    ...
    void IndicatorWindow::SyncVisibilityForState(::yip::IndicatorState s)
    {
        using S = ::yip::IndicatorState;
        const bool visible =
            (s == S::Recording || s == S::Saving || s == S::Expanded);
