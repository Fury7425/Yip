...
winrt::hstring FormatDuration(std::chrono::milliseconds ms)
{
    ...
    wchar_t buf[16];
    swprintf_s(buf, L"%02lld:%02lld", static_cast<long long>(mins),
...
namespace winrt::yip::viewmodels::implementation
{
    ...
    {
        const auto color = m_isRecording
    ...
    void MainViewModel::RefreshDevices()
    {
        ...
        for (auto& d : devices)
        {
            auto entry = winrt::make<DeviceEntry>(
                winrt::hstring{ d.id },
                winrt::hstring{ d.name },
        ...
        Raise(L"CanRecord");
        SetStatus(devices.empty() ? winrt::hstring{ L"No devices found" }
                                  : winrt::hstring{ L"Ready" });
    }
    ...
    void MainViewModel::RefreshRecordings()
    {
        ...
        for (const auto& it : fs::directory_iterator(folder, ec))
        {
            ...
            r.duration = std::chrono::milliseconds(
                sz > 44 ? (static_cast<int64_t>(sz - 44) * 1000) / (48000 * 2 * 4) : 0);
        ...
        for (auto& r : rows)
        {
            auto entry = winrt::make<RecordingEntry>(
                winrt::hstring{ r.path.wstring() },
                winrt::hstring{ r.path.filename().wstring() },
                FormatDuration(r.duration),
                FormatModified(r.modified));
    ...
    void MainViewModel::ToggleRecording()
    {
        if (m_isRecording)
        {
            ...
            if (status != REC_STATUS_OK)
            {
                ...
                if (err) {
                    ...
                    if (n > 0) { msg.resize(static_cast<size_t>(n) - 1);
        ...
        if (m_selectedDeviceIndex < 0
            || static_cast<uint32_t>(m_selectedDeviceIndex) >= m_devices.Size())
        {
    ...
    void MainViewModel::ApplySettings(winrt::hstring const& folder,
                                      uint32_t sampleRate,
                                      uint16_t channels)
    {
