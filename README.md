...
Native Windows audio recorder. Minimalist UI, lock-free WASAPI capture.
...
2. **`native` job** — depends on `rust`. Runs `cmake --preset release` + `cmake --build --preset release` (which msbuild-restores NuGet, then builds `yip-app.vcxproj`). Publishes `yip-app` as a self-contained artifact (`WindowsAppSDKSelfContained=true`, no runtime install required).
...
5. **`format-cpp` job** — `clang-format --dry-run -Werror` against `yip-app/`. Still `continue-on-error` pending the first formatting sweep.

NuGet packages and the cargo target dir are cached between runs.

## Milestones

See [CLAUDE.md](CLAUDE.md). Current: M1–M3 complete; M4 (indicator) complete; M5–M6 (VST) removed for now; M7 (polish) in progress.
